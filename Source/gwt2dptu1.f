...
C
C-----------------------------------------------------------------------
C6F5----READ HEAT PARAMETERS IF IHEAT IS ON
      IF(IHEAT.EQ.1) THEN
C6F6----READ AND PREPARE THE SOIL HEAT CAPACITY TERM
        DO K=1,NLAY
          NNDLAY = NODLAY(K)
          NSTRT = NODLAY(K-1)+1
          NDSLAY = NNDLAY - NODLAY(K-1)
...
20      FORMAT(80('-')/1X,'THE FOLLOWING ARRAYS ARE READ FOR SPECIES ',
     *    1X,'NUMBER ',I3/80('-')) 
C7B-----------READ STOCHIOMETRY OF SPATIALLY VARIABLE
        IF (ICHAIN.NE.0.AND.ISPRCT.NE.0) THEN      
...
C6A--------------CALCULATE DQ/DH
C              EPSS = 0.00001
C              CEPS = CONCIM(N,IPAREN)+EPSS
C              QEPS = CT * CEPS ** ETA
C              DQ = (QEPS - QA) / EPSS
CC              DQ = CT * ETA * CONCIM(N,IPAREN) ** (ETA-1.0)
CC              DTERMD = DTERMD + DQ
CC              RTERMD = RTERMD - QA + DQ*CONCIM(N,IPAREN)
              DTERMD = DTERMD + CT * CONCIM(N,IPAREN) **(ETA - 1.0)
            ENDIF
          ENDIF
        ENDIF
C-----------------------------------------------------------------------
